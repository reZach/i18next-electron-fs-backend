  ...
  _createClass(Backend, [{
    ...
    value: function write(writeQueue) {
      ...
      if (typeof i18nextElectronBackend !== "undefined") {
        ...
        var _loop2 = function _loop2(_i) {
          _this2.fs.readFile(toWork[_i].key, "utf8", function (error, data) {
            if (error) {
            ...
            _this2.fs.mkdir(root, {
              ...
              _this2.fs.writeFile(toWork[_i].key, JSON.stringify(data), function (error) {
                if (error) {
    ...
    value: function read(language, namespace, callback) {
      ...
      if (this.i18nextOptions.initImmediate === false) {
        try {
          var data = this.fs.readFileSync(filename, {
            encoding: "utf8"
          });
          callback(null, data);
        } catch (error) {
      ...
      if (typeof i18nextElectronBackend !== "undefined") {
        ...
        this.fs.readFile(filename, "utf8", function (error, data) {
          if (error) return callback(error, false);
